...
return client.sendImageAsSticker(from, base64imgnobg)
    .then(() => client.reply(from, `Here\'s your sticker \n\nProcessed for ${processTime(moment())} _Second_`))
                ...
                await client.reply(from, 'gambarnya ketinggalan😂', id)
            }
            break
        }
        case 'stikergif':
        ...
        case 'everyone':
            ...
            * Check premium feature at https://trakteer.id/red-emperor/showcase or chat Author for Information.
            */
            if (!isGroupMsg) return client.reply(from, 'cuma bisa digrup ajaa', id)
            if (!isGroupAdmins) return client.reply(from, 'cuma bisa dipake admin, jangan aneh aneh', id)
            const mentions = mentionList(sender.id, botNumber, groupMembers)
            await client.sendTextWithMentions(from, `woyy, ${pushname} memanggil kalian!!!\n\n${mentions}`)
            break
        case 'botstat': {
